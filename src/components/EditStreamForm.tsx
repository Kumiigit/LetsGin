...
import { Staff, StreamWithDetails, UpdateStreamPayload } from '../types';

interface EditStreamFormProps {
  stream: StreamWithDetails;
  staff: Staff[];
  onSave: (streamId: string, updateData: UpdateStreamPayload) => void;
  onClose: () => void;
}

export const EditStreamForm: React.FC<EditStreamFormProps> = ({
  stream,
  staff,
  onSave,
  onClose,
  ...
  const [startTime, setStartTime] = useState(stream.startTime);
  const [endTime, setEndTime] = useState(stream.endTime);
  const [description, setDescription] = useState(stream.description || '');
  const [streamLink, setStreamLink] = useState(stream.streamLink || '');
  
  // Get current assignments
  const currentCasters = stream.assignments.filter(a => a.role === 'caster').map(a => a.staffId);
  const currentObservers = stream.assignments.filter(a => a.role === 'observer').map(a => a.staffId);
  const currentProduction = stream.assignments.filter(a => a.role === 'production').map(a => a.staffId);
  
  const [selectedCasters, setSelectedCasters] = useState<string[]>(currentCasters);
  const [selectedObservers, setSelectedObservers] = useState<string[]>(currentObservers);
  const [selectedProduction, setSelectedProduction] = useState<string[]>(currentProduction);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (title && date && startTime && endTime && (selectedCasters.length > 0 || selectedProduction.length > 0)) {
      const updateData: UpdateStreamPayload = {
        title,
        date,
        startTime,
        endTime,
      ...
      onSave(stream.id, updateData);
      onClose();
    }
  };

  const toggleStaffSelection = (staffId: string, role: 'caster' | 'observer' | 'production') => {
    if (role === 'caster') {
      ...
      <div className="professional-card rounded-xl shadow-2xl w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-gray-800">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-lg font-semibold text-white">Edit Stream</h3>
          </div>
          <button
            onClick={onClose}
